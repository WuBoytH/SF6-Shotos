    ...
    if VarModule::is_flag(agent.module_accessor, ryu::status::flag::USED_DENJIN_CHARGE) {
        if macros::is_excute(agent) {
            if !VarModule::is_flag(agent.module_accessor, ryu::status::flag::SET_DENJIN_AURA) {
                ...
                sv_animcmd::EFFECT_STENCIL_ON(agent.lua_state_agent);
...
pub unsafe fn ryu_saving_aura_remover(agent: &mut L2CAgentBase) {
    if VarModule::is_flag(agent.module_accessor, ryu::status::flag::USED_DENJIN_CHARGE) {
        if macros::is_excute(agent) {
            ...
            agent.clear_lua_stack();
            sv_animcmd::EFFECT_STENCIL_OFF(agent.lua_state_agent);

            VarModule::off_flag(agent.module_accessor, ryu::status::flag::USED_DENJIN_CHARGE);
            VarModule::off_flag(agent.module_accessor, ryu::status::flag::SET_DENJIN_AURA);
        }
    }
}
